...
import { RiDeleteBin5Line, RiEyeLine, RiTicketLine } from "react-icons/ri";
...
const TicketTable = ({ rows, columns, fetchData, handleDeleteEvent, filters }: EventTableProps) => {
                ...
                <TableCell>
                  <Link href={`/events/${row.uuid}/edit`} key={row.id}>
                    <Box gap={3} style={{ display: 'flex', alignItems: 'center' }}>
                      ...
                      <Avatar sx={{ width: 50, height: 50 }} src={`${row.event_image}-xs.jpg`}><RiTicketLine/></Avatar>
