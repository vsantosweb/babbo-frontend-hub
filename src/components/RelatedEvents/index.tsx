...
export function RelatedEvents({ relatedEvents }: {relatedEvents: EventInterface[]}) {
    return (
        <Stack spacing={8}>
            <Styles.Container>
                <Styles.Header>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                        <button className={`slideNav el--prev`}><FaAngleLeft /></button>
                        <button className={`slideNav el--next`}><FaAngleRight /></button>
                    </div>
                </Styles.Header>
                ...
                >
                    {relatedEvents?.map((event, key) => (
                        <SwiperSlide key={key}>
                            <Link href={{ pathname: `/event/${event.slug}`, query: { id: event.uuid } }}>
                                <EventCard  {...event} />
                            </Link>
                        </SwiperSlide>
                    ))}
